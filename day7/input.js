...
const input = `mirrored silver bags contain 4 wavy gray bags.
...
const parseRule = (line) => {
  const rules = line.split("contain").map(trim);
  const [colorDef, containsDef] = rules;
  const color = colorDef.replace(" bags", "");

  const containsList = containsDef.replace(".", "").split(",");
  if (containsList[0] === "no other bags") return [color, []];
  const contains = containsList.map((r) => {
